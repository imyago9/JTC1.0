...
load_dotenv(resource_path('resources/dotenvs/parameters.env'))
...
def get_all_accounts(user_id, account_type=None):
    try:
        if account_type:
            cursor.execute("SELECT account_id, account_name, account_type "
                           "FROM accounts "
                           "WHERE user_id = %s "
                           "AND account_type = %s;", (user_id, account_type))
        else:
            cursor.execute("SELECT account_id, account_name, account_type "
                           "FROM accounts "
